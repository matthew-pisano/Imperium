...
public class Ai extends Player {
                        ...
                        if (getPlayerList().length != 0 && getGame() != null && System.currentTimeMillis() > lastRun + prefs.getInt("turnSpeed", 5000)) {
                            if (getCurrentPlayer() != null) {
                                if (getCurrentPlayer().getId() == id && !executing && !loneSurvivor() && getTurnNum() < MAX_TURNS) {
                                    ...
                                    if (getAllOwned().length == 0|| id == getGame().getLastAiId()){
                                        if(isHistorical() || getTurnNum() > getPlayerList().length) {
                                            Log.i("Ai cont", "Skipped: " + getNation().getName() + ", " + getAllOwned().length + ", " + getGame().getLastAiId() + ", " + id);
                                            changePlayer(true);
                                            continue;
                                        }
                                    }
                                    if (!getCurrentPlayer().isHuman()) {
                                        ...
                                        lastRun = System.currentTimeMillis();
                                        try {runLogic(); getGame().setLastAiId(id);} catch (Exception e) { e.printStackTrace(); }
    ...
    public boolean calcAcceptPeace(String enemy) {
        //return playerFromTag(enemy).getAllOwned().length < playerFromTag(enemy).getCoreProvs().length * .75;
    ...
    private void runLogic() { //executes turn stage logic in stage order
        ...
        if (getStage() == 0 || getStage() == 1) {
            if (isHistorical()) {
                ...
                if(diploList[3].size() == 0) searchAndDeclareWar();
            }
            ...
            if (style == BASIC) {
                ...
                /*if (style == MONGOL) {
                    if (getStage() == 0) mongolPlaceStage();
                    if (getStage() == 1) mongolAttackStage();
                }*/
            if (style == ROMAN) {
                ...
                /*if (style == HAN) {
                    if (getStage() == 0) hanPlaceStage();
                    if (getStage() == 1) hanAttackStage();
                }*/
                getAllStationedStacks();
            for(Province p : stationedIn){
    ...
    private void setupStage() {
        ...
        if (!getMap().allTaken()) {
            ...
            for (Province p : getMap().getList()) {
                if (p.getInterest() > max && p.getOwnerId() == -1) {
