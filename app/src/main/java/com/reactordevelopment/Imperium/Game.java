...
public class Game extends GameActivity implements java.io.Serializable {
    ...
    //public
    public static boolean isHistorical(){/*return year != 0;*/ return true;}
    ...
    public int getLastAiId(){return lastAiId;}
    ...
    public void setLastAiId(int id){Log.i("LastAi", "Now: "+id+", was: "+lastAiId); lastAiId = id;}
    ...
    public void updateAllOverlays(){
        ...
        for(int i=0; i<map.getList().length; i++) map.getList()[i].updateOverlays();
    ...
    private void update(){
        ...
        winCover.setLayoutParams(new ConstraintLayout.LayoutParams(screenHeight, screenWidth));
        winCover.animate().x(0).y(0).setDuration(0).start();
        winCover.setVisibility(View.INVISIBLE);
        winCover.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                winCover.animate().y(-1000).setDuration(1000);
                return false;
            }
        });
    ...
    protected boolean rollOut(final int[] rolls){
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(rolls[0] > rolls[3]) defeated.setRotation(0);
                else if(rolls[0] < rolls[3]) defeated.setRotation(180);
                else defeated.setBackgroundResource(R.drawable.tie);

                if(rolls[1] > rolls[4]) defeated2.setRotation(0);
                else if(rolls[1] < rolls[4]) defeated2.setRotation(180);
