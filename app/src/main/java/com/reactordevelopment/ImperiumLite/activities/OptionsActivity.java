...
import static com.reactordevelopment.ImperiumLite.activities.MainActivity.onOptions;
...
public class OptionsActivity extends AppCompatActivity {
    ...
    private void seekSet(){
        ...
        volScroll.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                int animProg = progress;
                if(animProg > 80) animProg = 80;
                animateX((int) (BAR_WIDTH*(-1+progress/100.0)), volBar, volAt);
                music = progress;
                edit = vars.edit();
    ...
    private void animateX(int to, ImageView bar, TextView at){
        ...
        if(to > -BAR_WIDTH+at.getWidth() && to < .8*at.getWidth())at.animate().x((float) (to+BAR_WIDTH-at.getWidth())).setDuration(0);
    }
